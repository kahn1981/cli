		...
		Context("for HTTP routes", func() {
			Context("when specifying --path", func() {
				It("reports an error with a minimum-version message", func() {
					...
					Eventually(session.Out).Should(Say(`FAILED`))
					Eventually(session.Err).Should(Say(`HTTP routes require CF API version 2\.36\.0 or higher. Your target is 2\.34\.0\.`))
					Eventually(session).Should(Exit(1))
				})
			})
		})
		...
		Context("for TCP routes", func() {
			Context("when specifying --port", func() {
				It("reports an error with a minimum-version message", func() {
					...
					Eventually(session.Out).Should(Say(`FAILED`))
					Eventually(session.Err).Should(Say(`TCP routes require CF API version 2\.53\.0 or higher. Your target is 2\.34\.0\.`))
					Eventually(session).Should(Exit(1))
				})
			})
	...
	Context("Flag Errors", func() {
		Context("when --hostname and --port are provided", func() {
			It("fails with a message about being unable to mix --port with the HTTP route options", func() {
				...
				Eventually(session.Err).Should(Say(`Cannot specify port together with hostname and/or path\.`))
				Eventually(session).Should(Exit(1))
			})
		})
		...
		Context("when --hostname and --random-port are provided", func() {
			It("fails with a message about being unable to mix --random-port with any other options", func() {
				...
				Eventually(session.Err).Should(Say(`Cannot specify random-port together with port, hostname and/or path\.`))
				Eventually(session).Should(Exit(1))
			})
		})
		...
		Context("when the space and domain exist", func() {
			...
			Context("when the route already exists", func() {
				...
				BeforeEach(func() {
					...
					Eventually(helpers.CF("create-domain", orgName, domainName)).Should(Exit(0))
					Eventually(helpers.CF("create-route", spaceName, domainName)).Should(Exit(0))
				})

				AfterEach(func() {
					Eventually(helpers.CF("delete-domain", domainName, "-f")).Should(Exit(0))
				})
				...
				It("warns the user that it has already been created and runs to completion without failing", func() {
					...
					Eventually(session.Out).Should(Say(`Route %s already exists\.`, domainName))
			...
			Context("when the route does not already exist", func() {
				Context("when the domain is private", func() {
					var privateDomainName string

					BeforeEach(func() {
						privateDomainName = "private-domain.com"
						Eventually(helpers.CF("create-domain", orgName, privateDomainName)).Should(Exit(0))
						Eventually(helpers.CF("create-route", spaceName, privateDomainName)).Should(Exit(0))
					})

					AfterEach(func() {
						Eventually(helpers.CF("delete-domain", privateDomainName, "-f")).Should(Exit(0))
					})

					Context("when no flags are used", func() {
						It("creates the route", func() {
							session := helpers.CF("create-route", spaceName, privateDomainName)
							Eventually(session.Out).Should(Say(`Creating route %s for org %s / space %s as %s\.\.\.`, privateDomainName, orgName, spaceName, userName))
							Eventually(session).Should(Exit(0))
						})
					})
					...
					Context("when the path is provided but the hostname is not", func() {
						...
						It("creates the route", func() {
							session := helpers.CF("create-route", spaceName, privateDomainName, "--path", path)
							Eventually(session.Out).Should(Say(`Creating route %s/%s for org %s / space %s as %s\.\.\.`, privateDomainName, path, orgName, spaceName, userName))
							Eventually(session).Should(Exit(0))
						})
					})
				})
				...
				Context("when the domain is shared", func() {
					...
					BeforeEach(func() {
						sharedDomainName = "shared-domain.com"
						Eventually(helpers.CF("create-shared-domain", orgName, sharedDomainName)).Should(Exit(0))
					})

					AfterEach(func() {
						Eventually(helpers.CF("delete-shared-domain", sharedDomainName, "-f")).Should(Exit(0))
					})

					Context("when no flags are used", func() {
						It("fails with an error message and exits 1", func() {
							session := helpers.CF("create-route", spaceName, sharedDomainName)
							Eventually(session.Out).Should(Say(`Creating route %s for org %s / space %s as %s\.\.\.`, sharedDomainName, orgName, spaceName, userName))
				...
				Context("when the domain is an HTTP domain", func() {
					var httpDomainName string

					BeforeEach(func() {
						httpDomainName = "http-domain.com"
						Eventually(helpers.CF("create-shared-domain", orgName, httpDomainName)).Should(Exit(0))
					})

					AfterEach(func() {
						Eventually(helpers.CF("delete-shared-domain", httpDomainName, "-f")).Should(Exit(0))
					})

					Context("when TCP flag options are provided", func() {
						It("fails with an error message and exits 1", func() {
							port := "90230"
							session := helpers.CF("create-route", spaceName, httpDomainName, "--port", port)
							Eventually(session.Out).Should(Say(`Creating route %s:%s for org %s / space %s as %s\.\.\.`, httpDomainName, port, orgName, spaceName, userName))
							Eventually(session.Err).Should(Say(`The route is invalid: host is required for shared-domains`))
							Eventually(session).Should(Exit(1))
						})
					})
					...
					Context("when the hostname is provided", func() {
						...
						Context("when no path is provided", func() {
							It("creates the route", func() {
								session := helpers.CF("create-route", spaceName, httpDomainName, "--hostname", hostName)
								Eventually(session.Out).Should(Say(`Creating route %s.%s for org %s / space %s as %s\.\.\.`, hostName, httpDomainName, orgName, spaceName, userName))
								Eventually(session).Should(Exit(0))
							})
						})
						...
						Context("when a path is provided", func() {
							It("creates the route", func() {
								...
								session := helpers.CF("create-route", spaceName, httpDomainName, "--hostname", "/some-host", "--path", path)
								Eventually(session.Out).Should(Say(`Creating route %s.%s%s for org %s / space %s as %s\.\.\.`, hostName, httpDomainName, path, orgName, spaceName, userName))
								Eventually(session).Should(Exit(0))
							})
						})
					})
					...
					Context("when the hostname is not provided", func() {
						...
						Context("when the path is provided", func() {
							It("fails with an error message and exits 1", func() {
								session := helpers.CF("create-route", spaceName, httpDomainName, "--path", path)
								Eventually(session.Out).Should(Say(`Creating route %s for org %s / space %s as %s\.\.\.`, httpDomainName, orgName, spaceName, userName))
								Eventually(session.Err).Should(Say(`The route is invalid: host is required for shared-domains`))
								Eventually(session).Should(Exit(1))
							})
						})
				...
				Context("when the domain is a TCP domain", func() {
					var tcpDomainName string

					BeforeEach(func() {
						tcpDomainName = "tcp-domain.com"
						Eventually(helpers.CF("create-shared-domain", orgName, tcpDomainName, "--router-group", "default-tcp")).Should(Exit(0))
					})

					AfterEach(func() {
						Eventually(helpers.CF("delete-shared-domain", tcpDomainName, "-f")).Should(Exit(0))
					})
					...
					Context("when HTTP flag options are provided", func() {
						It("fails with an error message and exits 1", func() {
							...
							path := helpers.PrefixedRandomName("path-")
							session := helpers.CF("create-route", spaceName, tcpDomainName, "--hostname", hostName, "--path", path)
							Eventually(session.Out).Should(Say(`Creating route %s.%s/%s for org %s / space %s as %s\.\.\.`, hostName, tcpDomainName, path, orgName, spaceName, userName))
					...
					Context("when a port is provided", func() {
						It("creates the route", func() {
							port := "1110"
							session := helpers.CF("create-route", spaceName, tcpDomainName, "--port", port)
							Eventually(session.Out).Should(Say(`Creating route %s:%s for org %s / space %s as %s\.\.\.`, tcpDomainName, port, orgName, spaceName, userName))
							Eventually(session).Should(Exit(0))
						})
					})
					...
					Context("when --random-port is provided", func() {
						It("creates the route", func() {
							session := helpers.CF("create-route", spaceName, tcpDomainName, "--random-port")
							Eventually(session.Out).Should(Say(`Creating route %s for org %s / space %s as %s\.\.\.`, tcpDomainName, orgName, spaceName, userName))
							Eventually(session.Out).Should(Say(`Route %s:\d+ has been created\.`, tcpDomainName))
							Eventually(session).Should(Exit(0))
						})
					})
				})
