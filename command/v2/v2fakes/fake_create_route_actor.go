...
type FakeCreateRouteActor struct {
	...
	CreateRouteWithExistenceCheckStub        func(orgGUID string, spaceName string, host string, path string, port types.NullInt, generatePort bool) (v2action.Route, v2action.Warnings, error)
...
func (fake *FakeCreateRouteActor) CreateRouteWithExistenceCheck(orgGUID string, spaceName string, host string, path string, port types.NullInt, generatePort bool) (v2action.Route, v2action.Warnings, error) {
	...
	fake.createRouteWithExistenceCheckArgsForCall = append(fake.createRouteWithExistenceCheckArgsForCall, struct {
		...
		port         types.NullInt
		generatePort bool
	}{orgGUID, spaceName, host, path, port, generatePort})
	fake.recordInvocation("CreateRouteWithExistenceCheck", []interface{}{orgGUID, spaceName, host, path, port, generatePort})
	fake.createRouteWithExistenceCheckMutex.Unlock()
	if fake.CreateRouteWithExistenceCheckStub != nil {
		return fake.CreateRouteWithExistenceCheckStub(orgGUID, spaceName, host, path, port, generatePort)
	}
	if specificReturn {
...
func (fake *FakeCreateRouteActor) CreateRouteWithExistenceCheckArgsForCall(i int) (string, string, string, string, types.NullInt, bool) {
	fake.createRouteWithExistenceCheckMutex.RLock()
	defer fake.createRouteWithExistenceCheckMutex.RUnlock()
	return fake.createRouteWithExistenceCheckArgsForCall[i].orgGUID, fake.createRouteWithExistenceCheckArgsForCall[i].spaceName, fake.createRouteWithExistenceCheckArgsForCall[i].host, fake.createRouteWithExistenceCheckArgsForCall[i].path, fake.createRouteWithExistenceCheckArgsForCall[i].port, fake.createRouteWithExistenceCheckArgsForCall[i].generatePort
}
